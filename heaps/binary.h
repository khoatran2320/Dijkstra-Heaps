...
private:
   void downHeap(int root){// starting with element in position int, sift it down the heap 
      ...
      do{
	      if (child < (elements-1)){
		      if ( array[child+1]->entry->key < array[child]->entry->key ) child++; // pick smaller
         }	   
	      if ( array[child]->entry->key < item->entry->key ){//swap
   ...
   void upHeap(int new_pos){// starting with element in position int, sift it up the heap
      ...
      while((new_pos != 0) && (item->entry->key < array[parent]->entry->key)) //loop while the item has not become the main root, and while its value is less than its parent
      {
